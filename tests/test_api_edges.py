...
def test_token_budget_exhaustion_returns_403(
    ...
    mock_llm_json(monkeypatch, prompt_tokens=3, completion_tokens=2)
...
def test_prelab_max_attempts_exhausted(client: TestClient) -> None:
    ...
    assert "max_attempts" in str(body.get("detail") or "").lower()
...
def test_soft_delete_survives_summary_failure(
    ...
    conv = container.conversations[cid]
...
def test_conversation_ttl_purge_on_health(client: TestClient) -> None:
    cid = _start(client)
    container = get_container()
    conv = container.conversations[cid]
    ...
    assert cid not in container.conversations
    assert client.get(f"/conversations/{cid}").status_code == 404
...
def test_idempotency_distinct_keys_call_llm_twice(
    ...
    mock = mock_llm_json(monkeypatch)
    cid = _start(client)
    r1 = client.post(
        f"/conversations/{cid}/messages",
